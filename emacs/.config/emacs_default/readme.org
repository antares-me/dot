  ...
  (use-package telega
    ...
    (progn
      ...
      (setq telega-use-docker t
            ...
            ;;telega-alert-mode t
            telega-language "ru"
            telega-proxies '((:server "127.0.0.1" :port 1443 :type '(:@type "proxyTypeMtproto" :secret "5f9e8a468b85abd1f647351a345fad32")))
            )
      ...
      ;;    (lambda ()
      ;;       (telega--addProxy '((:server "127.0.0.1" :port 1443 :type '(:@type "proxyTypeMtproto" :secret "dd5f9e8a468b85abd1f647351a345fad32"))) 'enable)))
      ;;(require 'telega-alert)
      (require 'telega-adblock)
      (require 'telega-dashboard)
      (setq dashboard-banner-length 90)
...
** AI
*** Codeium
Бесплатный, сверхбыстрый и расширяемый инструмент для автодополнения кода на основе искусственного интеллекта для Emacs
#+begin_src emacs-lisp :tangle no
  (use-package codeium
    :ensure t
    :init
    (add-to-list 'completion-at-point-function #'codeium-complete-at-point)
    :config
...
#+begin_src emacs-lisp
  (use-package gptel
    :ensure t
    :bind (("C-c g" . gptel-send)      ; Отправить выделенное или весь буфер
           ("C-c r" . gptel-rewrite))  ; Переписать выделенный регион
    :config
    (setq gptel-default-mode 'org-mode) ; Чтобы ответы красиво форматировались
...
#+begin_src emacs-lisp
  (use-package mcp
    ...
    :custom
    (mcp-hub-servers
     `(("ddg" . (:command "/home/antares/.local/bin/uvx"
  			:args	("duckduckgo-mcp-server")))
       ("go-mcp" . (:command "/home/antares/go/src/mcp/mcp" :args ()))
       ("git-mcp" . (:command "/home/antares/.local/bin/uvx" :args ("mcp-server-git", "--repository", "/home/antares/Work/registr")))
       ("bitrix" . (:url "https://mcp-dev.bitrix24.tech/mcp"))
       ("chrome" . (:command "npx" :args ("chrome-devtools-mcp@latest" "--autoConnect" "--channel=beta")))
       ("pandoc" . (:command "/home/antares/.local/bin/uvx" :args ("mcp-pandoc")))
       ;;("OpenCode" . (:command "/home/antares/.opencode/bin/opencode"
       ;;                        :args ("--model" "gemma4:e2b")))
       ))
    :hook (kill-emacs . mcp-hub-close-all-server)
    :config
    (require 'mcp-hub)
...
#+begin_src emacs-lisp
  (use-package ellama
    ...
    (setopt ellama-user-nick "Antares")
    (setopt ellama-assistant-nick "Jarvis")
    ...
    (setopt ellama-provider
            (make-llm-ollama
             ...
             :default-chat-non-standard-params '(("num_ctx" . 60000)))) ;;262144
    ...
    (setopt ellama-coding-provider
  	  (make-llm-ollama
  	   ...
  	   :embedding-model "nomic-embed-text"
  	   :default-chat-non-standard-params '(("num_ctx" . 10000))))
    ...
    :config
    (setq ellama-tools-enable-all t)
    (setq ellama-spinner-enabled t)
    (setq ellama-blueprints
   '((:act "Ассистент" :prompt
           "Ты — автономный ассистент. Твоя задача — выполнять действия, а не описывать их. Если есть доступный инструмент для создания файла или выполнения команды, ты *обязан* использовать его. Никогда не выдавай код или текстовое содержимое в ответе, если для этого есть инструмент. Используй инструменты молча."
           :for-devs t)))
    ;; show ellama context in header line in all buffers
    (ellama-context-header-line-global-mode +1)
    ;; show ellama session id in header line in all buffers
    (ellama-session-header-line-global-mode +1)
    (setq ellama-sessions-directory "~/.config/emacs_default/ellama-sessions")
    (setq ellama-skills-global-path "~/.config/emacs_default/ai/")
    ;; handle scrolling events
    (advice-add 'pixel-scroll-precision :before #'ellama-disable-scroll)
    (advice-add 'end-of-buffer :after #'ellama-enable-scroll))
#+end_src
*** CodeGPT
Пакет позволяет напрямую работать с OpenAI API
#+begin_src emacs-lisp :tangle no
  (use-package codegpt
    :ensure t
...
#+begin_src emacs-lisp
  (use-package aidermacs
    ...
    :config
    (setenv "OPENROUTER_API_KEY" (password-store-get "Personal/openrouter/api-key"))
    (setenv "OLLAMA_API_BASE" "http://127.0.0.1:11434")
    :custom
    (aidermacs-default-chat-mode 'code)
    (aidermacs-default-model "ollama/qwen2.5-coder:3b"))
#+end_src
*** ob-aider
#+begin_src emacs-lisp
  (use-package ob-aider
  :ensure (:host github :repo "localredhead/ob-aider.el")
  :after org
  :config
...
#+end_src
*** Copilot
Github Copilot временно отключен так как из России не работает
#+begin_src emacs-lisp :tangle no
  (use-package copilot
    :after '(track-changes)
    :ensure (:host github
...
#+end_src
*** Copilot-chat
Github Copilot Chat временно отключен так как из России не работает
#+begin_src emacs-lisp :tangle no
...
#+begin_src emacs-lisp :tangle no
  (use-package ollama
    :ensure (:host github :repo "nailuoGG/ollama.el" :branch "master" :depth 1
                   :files ("*.el" (:exclude "images/*"))))
#+end_src
...
#+begin_src emacs-lisp
  (use-package ollama-buddy
    ...
    (setq ollama-buddy-default-model "qwen3.5:9b"))
#+end_src
...
#+begin_src emacs-lisp
  (use-package minuet
  :ensure t                  ; Установить пакет, если он еще не установлен
  :bind (("M-i" . #'minuet-show-suggestion)        ; Показать подсказку (призрачный текст)
         ("M-m" . #'minuet-complete-with-minibuffer) ; Альтернативный режим (минибуфер)
         :map minuet-active-mode-map               ; Клавиши, когда подсказка активна
         ("M-p" . #'minuet-previous-suggestion)    ; Предыдущая подсказка
         ("M-n" . #'minuet-next-suggestion)        ; Следующая подсказка
         ("M-s" . #'minuet-accept-suggestion)      ; Принять всю подсказку
         ("M-a" . #'minuet-accept-suggestion-line)) ; Принять одну строку
  :init
  ;; Включаем автоматический режим для всех режимов программирования
  (add-hook 'prog-mode-hook #'minuet-auto-suggestion-mode)
  :config
  ;; --- Настройка для Ollama ---
  (setq minuet-provider 'openai-fim-compatible) ; Используем FIM API
  ;; Рекомендуется 1 для локальных моделей, чтобы экономить ресурсы
  (setq minuet-n-completions 1)
  ;; Ограничиваем контекст. Для 7b модели 2048 - хороший старт.
  (setq minuet-context-window 2048)

  ;; Настраиваем эндпоинт Ollama (стандартный порт 11434)
  (plist-put minuet-openai-fim-compatible-options :end-point "http://localhost:11434/v1/completions")
  ;; Плейсхолдер для API ключа (Ollama он не нужен)
  (plist-put minuet-openai-fim-compatible-options :api-key "TERM")
  ;; Указываем модель, которую вы скачали в Ollama
  (plist-put minuet-openai-fim-compatible-options :model "qwen2.5-coder:1.5b")
  ;; Настройка количества токенов (длина ответа)
  (minuet-set-optional-options minuet-openai-fim-compatible-options :max_tokens 256)
  ;; Небольшая задержка перед запросом, чтобы не дергать ИИ при каждом нажатии
...
#+begin_src emacs-lisp
  (use-package composer
    :ensure t
    ;;:config (progn
    ;;          (setq composer-use-managed-phar t
...
#+begin_src emacs-lisp
  (use-package org
    ...
    (progn
      ...
      (setq org-agenda-files '("~/Dropbox/org/Agenda.org" "~/Dropbox/org/webinar.org" "~/Dropbox/org/workTasks.org" "~/Dropbox/org/yandex-calendar.org"))
...
#+begin_src emacs-lisp
  (use-package org-modern-indent
    :ensure (:repo "jdtsmith/org-modern-indent"
             :fetcher github)
    :hook
...
#+begin_src emacs-lisp
  (use-package org-caldav
    :ensure t
    :config
    (setq org-caldav-url "https://caldav.yandex.ru/calendars/a.salnikov@notamedia.ru"
          org-caldav-username "a.salnikov@notamedia.ru"
          org-caldav-calendar-id "events-32542711"
          org-caldav-files nil
          org-caldav-inbox "~/Dropbox/org/yandex-calendar.org"
          org-caldav-sync-direction 'cal->org ;;'twoway
          org-icalendar-timezone "Europe/Saratov"
...
#+begin_src emacs-lisp
  (use-package org-yt
    :ensure (
             ...
             :fetcher github))
#+end_src
#+begin_src emacs-lisp :tanrle no
(require 'org)
(require 'org-element)
...
#+begin_src emacs-lisp
  (use-package git-sync-mode
    ...
    (add-to-list git-sync-allow-list '(~/Work/registr))
...
** TODO mu4e
#+begin_example bash
        $yay -S mu mbsync-git cyrus-sasl-xoauth2-git sasl-xoauth2-git
#+end_example
...
#+begin_src emacs-lisp
  (use-package websocket
    :ensure (:host github :repo "ahyatt/emacs-websocket"))
#+end_src
